...
if not hasattr(ev3_nengo, 'link'):
    ev3_nengo.link = ev3_nengo.ev3link.EV3Link('10.0.0.2')
link = ev3_nengo.link
...
with model:
    ...
    # ---------------------- NUCLEI ---------------------------------------
    lateral = nengo.Ensemble(n_neurons=100,dimensions=input_stim_dim) # a sparse representation of the input
    ...
    basal_out_redis = nengo.Node(basal_from_redis, size_out=2)
    
    #cortex = nengo.Ensemble(n_neurons=100,dimensions=emotion_dim)
    #basal = nengo.Ensemble(n_neurons=100,dimensions=emotion_dim)  # represents  current emotional state 
    ...
    #nengo.Connection(lateral, basal, eval_points=input_data, function=output_data,
    #                scale_eval_points=False, transform =per_input)

    nengo.Connection(lateral, basal_in_redis, eval_points=input_data, function=output_data,
                     scale_eval_points=False, transform=per_input)
    
    #nengo.Connection(lateral, basal_in_redis, function=joy2VA)
    ...
    #nengo.Connection(basal, central.input, transform = emotion_matrix)   # removed by Timmer
    #nengo.Connection(basal, central_in_redis, transform = emotion_matrix)   # added by Timmer for redis
    nengo.Connection(basal_out_redis, central_in_redis, transform=emotion_matrix)
    ...
    #nengo.Connection(lateral,central.input, function=fast_reaction)   # removed by Timmer
    nengo.Connection(lateral, central_in_redis, function=fast_reaction)   # added by Timmer for redis
    ...
    #nengo.Connection(central.output, movement)   # removed by Timmer
    nengo.Connection(central_out_redis, movement)   # added by Timmer
