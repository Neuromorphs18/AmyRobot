...
with model:
    ...
    def central_from_redis(t):
        ...
        cfr3 = float(r.get('WTA_out_3'))
        movement_vec = [cfr0, cfr1, cfr2, cfr3]
        print (movement_vec)
        return movement_vec
    central_in_redis = nengo.Node(central_2_redis, size_in=4)
    central_out_redis = nengo.Node(central_from_redis, size_out=4)
    central = nengo_spa.networks.selection.WTA(n_neurons=100, n_ensembles=emotional_states,
                                               threshold=0.3, 
                                               function=lambda x: 1 if x>0 else 0)
    PFC = nengo.Ensemble(n_neurons=100,dimensions=emotion_dim)
